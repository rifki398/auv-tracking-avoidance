function [xdot,U,M] = remus100(x,ui,Vc,betaVc,alphaVc,w_c)
...
u_c = Vc * cos( betaVc - psi ) * cos(alphaVc - theta);                               
v_c = Vc * sin( betaVc - psi ) * cos(alphaVc - theta);
w_c = Vc * sin(alphaVc - theta);

nu_c = [u_c v_c w_c 0 0 0]'; % Ocean current velocities
% Dnu_c = [nu(6)*v_c -nu(6)*u_c 0 0 0 0]'; % Time derivative of nu_c
Dnu_c = [Smtrx(x(4:6)) zeros(3);
    zeros(3) zeros(3)] * nu_c;
